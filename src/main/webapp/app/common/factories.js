...
factories.factory('authFactory', ['$q', '$location', 'authInfo', 'usersAPI', 'accountAPI',
    function ($q, $location, authInfo, usersAPI, accountAPI) {
        var dataFactory = {};
        ...
        dataFactory.loadRememberedSession = function () {
            ...
            if (typeof (Storage) !== 'undefined') {
                ...
                if (email && token && email !== 'undefined' && token !== 'undefined') {
                    ...
                    usersAPI.findByEmail(authInfo.email, authInfo.token)
                            ...
                            .error(function (error) {
                                alert('Saved session has expired, please login.');
                                $location.path('/login');
                                deferred.reject();
                            });
                } else {
        ...
        dataFactory.forgetSession = function () {
            if (typeof (Storage) !== 'undefined') {
                accountAPI.postLogout(authInfo.user);
                // Back up the important information about the session
        ...
        dataFactory.resolveIsLoggedIn = function () {
            ...
            if (authInfo.token === undefined || authInfo.email === undefined)
                authenticated = false;

...
batches.controller('batchesCtrl', ['$scope', '$location', '$q', 'batchesAPI', 'tablesAPI', 'authInfo', function ($scope, $location, $q, batchesAPI, tablesAPI, authInfo) {
        ...
        $scope.reloadBatches = function () {
            $scope.loaded = false;
            $scope.requestFailed = false;
            batchesAPI.getAll(authInfo.token)
...
batches.controller('batchViewUsersCtrl', ['$scope', '$location', '$routeParams', '$q', 'batchesAPI', 'usersAPI', 'authInfo', function ($scope, $location, $routeParams, $q, batchesAPI, usersAPI, authInfo) {
        ...
        $scope.addSelected = function () {
            ...
            for (var i = 0; i < selectedRows.length; i++) {
                promises.push(batchesAPI.addUser(authInfo.token, $routeParams.batch_id, selectedRows[i]).error(function(res) {
                    alert(res);
                }));
            }
...
batches.controller('batchCreateCtrl', ['$scope', '$location', 'tablesAPI', 'batchesAPI', 'authInfo', function ($scope, $location, tablesAPI, batchesAPI, authInfo) {
        ...
        batchesAPI.getAssignmentTypes(authInfo.token).success(function (res) {
            $scope.assignment_type_dd_items = res;
            $scope.loaded = true;
            $scope.requestFailed = false;
        });
        $scope.setAssignmentType = function (assignmentTypeObj) {
            $scope.assignment_type = assignmentTypeObj;
        };
        $scope.create = function () {
            ...
            batchObj.assignmentTypeID = $scope.assignment_type.id;
            batchObj.assignmentDescription = $scope.assignment_type.description;
